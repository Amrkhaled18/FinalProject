...
const NearbyPlaces = () => {
    ...
    // FIXED: Moved fetchPlaces outside of useEffect to avoid hook call issues
    const fetchPlaces = async () => {
        ...
        try {
            ...
            if (res.success && res.data) {
                ...
                // Apply client-side filtering - updated to match database structure
                ...
                if (filtered.length > 0) {
                    console.log('Sample place data:', filtered[0]);
    ...
    // FIXED: Single useEffect that calls fetchPlaces
    useEffect(() => {
        fetchPlaces();
    ...
    // Handle hotel booking link - renamed for clarity
    const handleHotelDetails = (place) => {
        // Hotels use detailURL for booking.com links
        ...
        console.log('Hotel details debug:', {
            ...
            DetailURL: place.DetailURL,
            finalLink: detailsLink
        });

        if (detailsLink) {
            console.log('Opening hotel details link:', detailsLink);
            window.open(detailsLink, '_blank');
        } else {
            console.warn('No details link found for:', place.name || place.Name);
            alert('Booking details not available for this hotel');
        }
    };

    // Handle getting directions/location - updated to use googleMapsLink
    const handleGetLocation = (place) => {
        // Priority: Use googleMapsLink if available, otherwise fall back to address search
        if (place.googleMapsLink && place.googleMapsLink.trim()) {
            window.open(place.googleMapsLink, '_blank');
        } else {
            // Fallback to address search if no googleMapsLink
        ...
        console.log('Directions debug:', {
            ...
            fallbackAddress: place.address || place.Address
        });
    };

    // Handle monument more info link
    const handleMonumentInfo = (place) => {
        // Check various possible field names for monument info links - note the correct casing
        ...
        console.log('Monument info debug:', {
            placeName: place.name || place.Name,
            detailURL: place.detailURL,
            DetailURL: place.DetailURL,
            finalLink: infoLink
    ...
    // Handle restaurant/cafe social link (Facebook, etc.)
    const handleSocialLink = (place) => {
        // Check various possible field names for social/website links
        ...
        console.log('Social link debug:', {
            placeName: place.name || place.Name,
            detailURL: place.detailURL,
            DetailURL: place.DetailURL,
            finalLink: socialLink
        });
    ...
    // Function to get the correct image URL - updated to match database fields
    const getImageUrl = (place) => {
        let imageSource = null;

        // Check for image fields that match your database structure
        if (place.imageURL && place.imageURL.trim()) {
            imageSource = place.imageURL;
        }
        // Check for ImageURL (uppercase I)
        else if (place.ImageURL && place.ImageURL.trim()) {
            imageSource = place.ImageURL;
        }
        // Check if there's an images array (like in monuments)
        else if (place.images && place.images.length > 0 && place.images[0].imageURL) {
            imageSource = place.images[0].imageURL;
        }
        // Then try other possible field names
        else if (place.image_1) {
            imageSource = place.image_1;
        }
        // Then try generic image field
        else if (place.image) {
        ...
        // Finally use fallback
            ...
            const placeName = place.name || place.Name || place.hotel_name || '';
    ...
    // Get place name based on database structure - fixed field names
    const getPlaceName = (place) => {
        // Use correct field name from CSV: Name (uppercase N)
        return place.Name || place.name || place.hotel_name || 'Unnamed Place';
    };

    // Format rating display - updated to match database fields
    ...
    // Check if place has social/info links - updated for database structure
    const hasExternalLink = (place) => {
        let hasDetailURL = false;

        // All categories use detailURL (lowercase d) - prioritize this first
        ...
        // Debug logging
        console.log('Checking external link for:', place.name || place.Name, {
            category: selectedCategory,
            detailURL: place.detailURL,
            DetailURL: place.DetailURL,
            hasDetailURL: hasDetailURL,
            detailURL_exists: !!place.detailURL,
            detailURL_empty: !place.detailURL || !place.detailURL.trim()
        });
    ...
    return (
                    ...
                    <div style={{
                        ...
                        {places.map((place, index) => (
                            <div key={place.id || place.hotel_id || `place-${index}`} style={{
                                ...
                                {/* Category-specific button configurations */}
                                {selectedCategory === 'hotels' ? (
                                    // Hotels: Show Details button only if detailURL exists, always show Directions
                                    hasExternalLink(place) ? (
                                        <div style={{ display: 'flex' }}>
                                            <button
                                                onClick={() => handleHotelDetails(place)}
                                                style={{
                                        ...
                                        // Only show Directions button if no booking details available
                                        <button
                                            onClick={() => handleGetLocation(place)}
                                            style={{
                                                backgroundColor: '#4A00E0',
                                    ...
                                    // Monuments: Two buttons side by side - More Info + Get Directions
                                    <div style={{ display: 'flex' }}>
                                        {hasExternalLink(place) && (
                                            <button
                                                onClick={() => handleMonumentInfo(place)}
                                                style={{
                                        ...
                                        <button
                                            ...
                                            style={{
                                                ...
                                                border: 'none',
                                                padding: '12px',
                                                flex: hasExternalLink(place) ? '1' : '2',
                                                fontSize: '15px',
                                                fontWeight: '600',
                                                cursor: 'pointer',
                                                transition: 'background-color 0.2s ease',
                                    ...
                                    // Restaurants & Cafes: Two buttons side by side - Visit Page + Get Directions
                                    <div style={{ display: 'flex' }}>
                                        {hasExternalLink(place) && (
                                            <button
        ...
        /* Card hover effects */
        .nearby-places-page > div:nth-child(4) > div:nth-child(2) > div > div:hover {
