...
const Home = () => {
    ...
    // Fetch places data from API with better error handling
    useEffect(() => {
    ...
    // Fetch first images with better error handling
    useEffect(() => {
    ...
    const generateRecommendations = () => {
        ...
        const scoredPlaces = allPlaces.map(place => {
            let score = (place.Attributes || place.rating || 0) * 10; // Base score from rating

            // Boost score based on behavior
            const placeId = place.id || place.hotel_id || place.locationID || place.LocationID;
            if (trackedBehavior.clicks && trackedBehavior.clicks[placeId]) {
                score += trackedBehavior.clicks[placeId] * 5;
            }

            if (trackedBehavior.views && trackedBehavior.views[placeId]) {
                score += trackedBehavior.views[placeId] * 2;
            }

            // Boost score if category matches preferences
            if (userPreferences.preferredCategories.includes(getCategoryName(place))) {
                score += 15;
            }
    ...
    }, [userPreferences, windowWidth, allPlaces]);
    ...
    const trackPlaceInteraction = (place, interactionType) => {
        ...
        if (!placeId) {
            console.warn('Cannot track interaction - no valid place ID found');
            return;
        }
    ...
    // Get top-rated places across all categories with improved filtering
    useEffect(() => {
        if (!Array.isArray(allPlaces) || allPlaces.length === 0) {
            setFeaturedPlaces([]);
            return;
        ...
        const topPlaces = placesToShow
            .sort((a, b) => {
                const ratingA = a.rating || a.Attributes || 0;
                const ratingB = b.rating || b.Attributes || 0;
                return ratingB - ratingA;
            })
            .slice(0, numFeatured);

        setFeaturedPlaces(topPlaces);
    }, [activeCategory, windowWidth, allPlaces]);
    ...
    const getPriceRange = (priceLevel) => {
        return PRICE_RANGES[priceLevel] || 'N/A';
    };
    ...
    // Get correct category name with improved mapping
    const getCategoryName = (place) => {
        // Check if place has a category field
        if (place.category) {
            return place.category;
        }

        // Check backend Category field and map to frontend categories
        if (place.Category) {
        ...
        // Check if it's a hotel based on hotel_name
        if (place.hotel_name) {
            return 'hotels';
        }

        // Fallback
        return activeCategory !== 'all' ? activeCategory : 'monuments';
    };
                ...
                <div className="section-container">
                    ...
                    <div className="service-cards">
                        {featuredPlaces.map((place, index) => (
                            <div
                                ...
                                key={place.id || place.hotel_id || place.locationID || place.LocationID || `place-${index}`}
                            ...
                            >
                                <div className="card-image-container">
                                    <img
                                        ...
                                        onError={(e) => {
                                            ...
                                            e.target.src = `https://source.unsplash.com/300x200/?${getCategoryName(place)},cairo`;
                                        }}
                                    />
                                    ...
                                    <div className="rating-badge">
                                        ...
                                        <span className="rating-number">{formatRating(place)}</span>
                                    </div>
                                </div>
                                ...
                                <div className="card-content">
                                    ...
                                    <div className="location-info">
                                        ...
                                        <span>{place.address || place.Address || place.distance || 'Cairo, Egypt'}</span>
                                    </div>
                    ...
                    {featuredPlaces.length === 0 && !loading && (
                        <div style={{
                            ...
                            <p>No places found for the selected category. Try selecting "All" or check back later.</p>
                        </div>
                    )}
                            ...
                            <div className="service-cards recommendation-cards">
                                {recommendations.map((place, index) => (
                                    <div
                                        ...
                                        key={place.id || place.hotel_id || place.locationID || place.LocationID || `rec-${index}`}
                                            ...
                                            {windowWidth > 480 && (
                                                <div className="ai-reason">
                                                    ...
                                                    <span className="ai-reason-text">
                                                        {getCategoryName(place) === 'restaurants' ? 'Matches your dining preferences' :
                                                            getCategoryName(place) === 'cafes' ? 'Similar to cafes you viewed' :
                                                                getCategoryName(place) === 'hotels' ? 'Based on your price range' :
            ...
            <section className="features-section">
                <div className="section-container">
                    ...
                    <div className="features-grid">
                        ...
                        <div className="feature-card feature-animate delay-100">
                            <div className="feature-hover-effect"></div>
                            <div className="feature-icon">
                                <svg xmlns="http://www.w3.org/2000/svg" width="40" height="40" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                            ...
                            <h3 className="feature-title">AI Assistant Chat</h3>
                            <p className="feature-description">
                                Get instant answers, directions, and local insights with our intelligent chatbot available 24/7.
                            </p>
                        </div>

                        <div className="feature-card feature-animate delay-200">
                            <div className="feature-hover-effect"></div>
                            <div className="feature-icon">
                                <svg xmlns="http://www.w3.org/2000/svg" width="40" height="40" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                            ...
                            <h3 className="feature-title">Reliable Navigation</h3>
                            <p className="feature-description">
                                Get precise directions to any destination in Cairo with real-time updates and offline support.
                            </p>
                        </div>

                        <div className="feature-card feature-animate delay-300">
                            <div className="feature-hover-effect"></div>
                            <div className="feature-icon">
                                <svg xmlns="http://www.w3.org/2000/svg" width="40" height="40" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                        ...
                        <div className="feature-card feature-animate delay-400">
                            <div className="feature-hover-effect"></div>
                            <div className="feature-icon">
                                <svg xmlns="http://www.w3.org/2000/svg" width="40" height="40" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                            ...
                            <h3 className="feature-title">Cultural Insights</h3>
                            <p className="feature-description">
                                Learn about the rich history and cultural significance of each monument and landmark.
                            </p>
                        </div>

                        <div className="feature-card feature-animate delay-500">
                            <div className="feature-hover-effect"></div>
                            <div className="feature-icon">
                                <svg xmlns="http://www.w3.org/2000/svg" width="40" height="40" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
